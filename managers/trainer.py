...
class Trainer():
    ...
    def train_epoch(self):
        ...
        for b_idx, batch in bar:
            ...
            # BCELoss
            m = nn.Sigmoid()
            score_pos = torch.squeeze(m(score_pos), dim=-1)
    ...
    def train(self):
        ...
        for epoch in range(1, self.params.num_epochs + 1):
            ...
            logging.info(
                f'Epoch {epoch} with loss: {train_loss}, training auc: {train_auc}, training aupr: {train_aupr}')
    ...
    def make_valid_test(self):
        ...
        if result['auc'] >= self.best_metric:
            ...
            logging.info(
                '\033[93m Test Performance Per Class:' + str(test_result) + 'in ' + str(time.time() - tic) + '\033[0m')
        else:
            self.not_improved_count += 1
            if self.not_improved_count > self.params.early_stop:
                logging.info(
                    f"Validation performance didn\'t improve for {self.params.early_stop} epochs. Training stops.")
                logging.info('\033[93m Test Performance Per Class:' + str(self.best_test_result) + 'in ' + str(
                    time.time() - tic) + '\033[0m')
